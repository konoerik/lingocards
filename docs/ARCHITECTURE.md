...
│   └── style.css
├── images/
│   └── <category>/         # WebP illustrations (512px, q85) generated by generate_images.py
├── audio/
...
│   └── webkit/
└── scripts/
    ├── generate_images.py      # gpt-image-1 / DALL-E image generation → WebP 512px
    ├── convert_to_webp.py      # bulk PNG→WebP conversion utility (one-time migration)
...
- **WebP at 512px for all card images** — all card illustrations are stored as WebP quality 85 at 512×512px. This covers the largest display slot (260px CSS × 2× DPR = 520px) on all target devices with no visible upscaling. Result: 100MB → 3.5MB (97% reduction) vs original 1024px PNGs. No `<picture>` fallback needed; WebP is supported on all target devices (iOS 14+, Chrome, Safari 14+).
...
[
  {
    ...
    "level": 1,                               // 1 = Beginner, 2 = Intermediate, 3 = Advanced — drives difficulty dot + filter
    "audio": {
