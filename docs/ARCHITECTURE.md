...
- **Image generation via OpenRouter** — `scripts/generate_images.py` calls `google/gemini-2.5-flash-image` via OpenRouter chat completions with `modalities: ["image"]`; saves base64-decoded PNG; skips existing files; updates words.json with image paths. Model override via `MODEL` env var, category filter via `CATEGORY` env var.
- **No SRS / no backend** — navigation is simple prev/next with optional shuffle; no spaced-repetition or progress tracking; keeps the app suitable for a 3.5-year-old and eliminates backend complexity
