...
- [ ] If `app.js`, `style.css`, `manifest.json`, or any `data/decks/*.json` changed: bump the cache version string in `sw.js` (e.g. `lingocards-v1` → `lingocards-v2`) so existing users get fresh files on next visit
